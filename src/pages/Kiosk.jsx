    ...
    <div className="min-h-screen bg-[#1a1a1a] text-white font-['Cairo'] relative overflow-x-hidden selection:bg-[#ff6b00] selection:text-white" dir="rtl">
      ...
      {phase === 2 && (
        <div className="absolute inset-0 bg-[#141414] flex flex-col z-10 animate-fadeIn animate-duration-300">
          ...
          <div className="p-6 flex justify-between items-center border-b border-white/5 bg-[#1a1a1a]">
            <button 
              onClick={() => setPhase(1)}
              className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-lg cursor-pointer"
            >
      ...
      {phase === 3 && (
        <div className="min-h-screen flex flex-col bg-white text-black animate-fadeIn animate-duration-300" dir="rtl">
          {/* رأس */}
          <div className="flex items-center justify-between px-5 pt-5 pb-3 shrink-0 border-b border-gray-100 bg-white">
            <button type="button" onClick={() => setPhase(2)}
              className="flex h-9 w-9 items-center justify-center rounded-full bg-gray-100 text-gray-700 hover:bg-gray-200 cursor-pointer">✕</button>
            <div className="flex items-center gap-2 text-right">
              <div>
                <h2 className="text-gray-900 text-lg font-black">طلب جديد</h2>
                <p className="text-xs font-black mt-0.5 text-[#ff6b00]">{orderType === 'dine-in' ? 'أكل في المطعم 🍽️' : 'سفري 🥡'}</p>
              </div>
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#ff6b00]/10 text-[#ff6b00]">🍴</div>
            </div>
          </div>
          {/* أزرار الأحجام */}
            ...
            <div className="grid grid-cols-4 gap-3">
              {[{ id: 'all', label: 'الكل' },{ id: 'L', label: 'L' },{ id: 'XL', label: 'XL' },{ id: 'XXL', label: 'XXL' }].map(sz => (
                <button key={sz.id} type="button" onClick={() => setActiveSize(sz.id)}
                  className={`rounded-2xl border-2 py-5 text-2xl font-black transition-all cursor-pointer ${activeSize === sz.id ? 'bg-[#ff6b00] text-white border-[#ff6b00] shadow-xl shadow-[#ff6b00]/40 scale-[1.02]' : 'bg-gray-50 text-gray-800 border-gray-300 hover:bg-white hover:border-[#ff6b00]/50'}`}>
                  {sz.label}
                </button>
              ))}
            </div>
                    ...
                    {filteredProducts.map(p => {
                      ...
                      if (hasSizes) {
                        ...
                        if (sizesToShow.length === 1) {
                          ...
                          return (<button key={p.id+sz.label} type="button" onClick={() => addItem(p.id, sz.label, sz.price)} className={`relative rounded-2xl border-2 p-2.5 text-right transition-all cursor-pointer ${qty > 0 ? 'border-[#ff6b00] bg-[#ff6b00]/[0.07]' : 'border-gray-200 bg-white hover:border-[#ff6b00]/40'}`}>
                            ...
                            <p className="text-[#ff6b00] text-sm font-black mt-1">{formatCurrency(sz.price)}</p>
                            {qty > 0 && (<div className="absolute top-2 left-2 flex items-center gap-1 bg-[#ff6b00] text-white rounded-full px-1.5 py-0.5">
                        ...
                        return (<div key={p.id} className="rounded-2xl border-2 border-gray-200 bg-white p-3 cursor-pointer hover:border-[#ff6b00]/40 transition-all flex flex-col justify-between">
                          <div>
                            <div className="aspect-[4/3] rounded-xl bg-gray-50 mb-2 overflow-hidden flex items-center justify-center"><TransparentImg src={p.image || fallbackImg(p.category)} alt={p.name} className="w-full h-full object-cover" /></div>
                          ...
                          <div className="flex flex-col gap-2">
                            {sizesToShow.map(sz => { const key = `${p.id}__${sz.label}`; const qty = cart[key]?.qty || 0; return (
                              <button key={sz.label} type="button" onClick={() => addItem(p.id, sz.label, sz.price)} className={`flex items-center justify-between gap-2 rounded-xl border px-3 py-2.5 transition-all active:scale-[0.98] cursor-pointer ${qty > 0 ? 'border-[#ff6b00] bg-[#ff6b00]/10 font-bold text-[#ff6b00]' : 'border-gray-200 bg-gray-50 hover:bg-white text-gray-700'}`}>
                                <span className="text-[#ff6b00] text-sm font-black">{formatCurrency(sz.price)}</span>
                      ...
                      return (<button key={p.id} type="button" onClick={() => addItem(p.id)} className={`relative rounded-2xl border-2 p-2.5 text-right transition-all cursor-pointer ${qty > 0 ? 'border-[#ff6b00] bg-[#ff6b00]/[0.07]' : 'border-gray-200 bg-white hover:border-[#ff6b00]/40'}`}>
                        <div className="aspect-[4/3] rounded-xl bg-gray-50 mb-2 overflow-hidden flex items-center justify-center"><TransparentImg src={p.image || fallbackImg(p.category)} alt={p.name} className="w-full h-full object-cover" /></div>
                        <p className="text-gray-900 font-bold text-sm truncate leading-snug">{p.name}</p>
                        <p className="text-[#ff6b00] text-sm font-black mt-1">{formatCurrency(p.price)}</p>
                        {qty > 0 && (
                          <div className="absolute top-2 left-2 flex items-center gap-1 bg-[#ff6b00] text-white rounded-full px-1.5 py-0.5">
          ...
          <div className="px-5 pt-3 pb-5 shrink-0 border-t border-gray-200 bg-gray-50/50 space-y-2">
            <textarea
              ...
              rows={1}
              className="w-full rounded-xl bg-white border border-gray-200 px-3 py-2 text-gray-900 text-sm outline-none placeholder:text-gray-400 text-right resize-none focus:border-[#ff6b00]/50"
            />
            {cartItemCount > 0 && (
              <div className="flex items-center justify-between rounded-2xl bg-[#ff6b00]/10 border border-[#ff6b00]/30 px-4 py-2.5">
                <span className="text-[#ff6b00] font-black text-xl">{formatCurrency(cartTotal)}</span>
                <span className="text-gray-700 text-sm font-bold">{cartItemCount} صنف</span>
              </div>
            )}
            <button
              type="button"
              onClick={() => { if (cartItemCount > 0) setPhase(4); }}
              disabled={cartItemCount === 0}
              className="w-full rounded-2xl bg-gradient-to-l from-[#ff6b00] to-[#e53935] py-5 text-white font-black text-xl shadow-lg shadow-[#ff6b00]/30 disabled:opacity-40 active:scale-[0.98] transition-transform cursor-pointer"
            >
              تأكيد الطلب وإنشائه ✓
            </button>
          </div>
      ...
      {phase === 4 && (
        <div className="min-h-screen bg-[#141414] py-12 px-6 flex flex-col items-center justify-center animate-fadeIn animate-duration-300">
          <div className="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            ...
            <div className="lg:col-span-5 bg-[#1e1e1e] border border-white/5 rounded-3xl p-6 flex flex-col justify-between space-y-6">
              ...
              <div className="border-t border-white/5 pt-4 space-y-3.5 mt-auto">
                ...
                {note && (
                  <div className="flex justify-between text-xs text-gray-400">
                    <span>الملاحظة:</span>
                    <span className="font-bold text-[#ff6b00] truncate max-w-[180px]">{note}</span>
                  </div>
                )}
                <div className="flex justify-between text-xs text-gray-400">
            ...
            <div className="lg:col-span-7 bg-[#1e1e1e] border-2 border-[#ff6b00]/30 rounded-3xl p-8 relative overflow-hidden shadow-2xl flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between mb-6 border-b border-white/5 pb-4">
                  <div>
                    <h3 className="text-2xl font-black text-white flex items-center gap-2">
                      ...
                      جهاز الدفع الإلكتروني TPE
                    </h3>
                    <p className="text-gray-400 text-xs mt-1">الرجاء استخدام بطاقتك الذهبية أو CIB لتأكيد الطلب</p>
                  </div>
                  <div className="bg-[#ff6b00] text-white font-extrabold px-3 py-1.5 rounded-xl text-[10px] flex items-center gap-1.5 shadow-md">
                ...
                <div className="flex flex-col md:flex-row items-center gap-6 bg-[#141414] p-6 rounded-2xl border border-white/5">
                  ...
                  {/* مجسم جهاز TPE بالـ CSS */}
                  <div className="w-48 bg-[#2d3238] rounded-3xl p-4 border-4 border-gray-600 shadow-2xl relative flex flex-col gap-3 shrink-0 transform hover:scale-105 transition-transform duration-300">
                    {/* قارئ البطاقات من الأعلى */}
                    <div className="w-36 h-2 bg-gray-800 rounded mx-auto mb-1"></div>
                    
                    {/* شاشة الـ TPE المضيئة باللون الأخضر/الأزرق الكلاسيكي */}
                    <div className="w-full bg-[#82c91e]/90 text-black font-mono p-3 rounded-lg border-2 border-black h-24 flex flex-col justify-between text-[10px] font-bold leading-tight select-none shadow-inner">
                      {tpeStep === 1 && (
                        <>
                          <div className="text-center font-bold tracking-wider text-xs border-b border-black/25 pb-1">OMEGA PAY</div>
                          <div className="text-center mt-2 animate-pulse">أدخل البطاقة 💳</div>
                          <div className="text-right text-xs mt-1 text-black/80">{finalTotal}.00 DZD</div>
                        </>
                      )}
                      {tpeStep === 2 && (
                        <>
                          <div className="text-center border-b border-black/25 pb-1">أدخل الرمز السري</div>
                          <div className="text-center text-lg tracking-widest mt-1">
                            {pinDigits.split('').map(() => '*').join(' ') || '_ _ _ _'}
                          </div>
                          <div className="text-left text-[9px] text-black/70">اضغط OK للتأكيد</div>
                        </>
                      )}
                      {tpeStep === 3 && (
                        <>
                          ...
                          <div className="flex justify-center items-center mt-2">
                            <span className="w-5 h-5 border-2 border-t-black border-r-transparent border-b-black border-l-transparent rounded-full animate-spin"></span>
                          </div>
                          <div className="text-center text-[8px] text-black/70 mt-1">يرجى الانتظار</div>
                        </>
                      )}
                      {tpeStep === 4 && (
                        <>
                          <div className="text-center text-xs font-black text-green-900">عملية مقبولة ✓</div>
                          <div className="text-center text-[8px] mt-1">تم سحب {finalTotal} د.ج</div>
                          <div className="text-center font-bold mt-1 text-[9px] bg-green-900/10 rounded">شكراً لكم</div>
                        </>
                      )}
                    </div>
                  ...
                  <div className="flex-1 space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="pt-2">
                          <button
                            ...
                            className="px-4 py-2 bg-gradient-to-r from-[#ff6b00] to-[#ff9e00] text-white font-bold text-xs rounded-xl shadow-md cursor-pointer hover:shadow-lg transition-all animate-bounce"
                          >
                    ...
                    {tpeStep === 2 && (
                      <div className="space-y-2">
                        ...
                        <p className="text-gray-400 text-xs leading-relaxed">
                          اكتب الرمز السري المتكون من 4 أرقام على لوحة مفاتيح الـ TPE لتفويض خصم مبلغ <span className="text-[#ff6b00] font-extrabold">{finalTotal} د.ج</span> من رصيدكم.
                        </p>
              ...
              <div className="thermal-receipt rounded-2xl p-6 text-black space-y-6 shadow-2xl relative overflow-hidden" id="print-receipt-section">
                ...
                <div className="bg-gray-50 p-2.5 rounded-lg text-[9px] font-mono space-y-1 text-gray-600 border border-gray-150">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold">بطاقة بنكية (TPE)</span>
                  </div>
                  <div className="flex justify-between">
                    <span>حالة المعاملة:</span>
                    <span className="font-bold text-green-600">مقبولة بنجاح</span>
                  </div>
                  <div className="flex justify-between">
