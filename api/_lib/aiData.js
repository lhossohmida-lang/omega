// AI data gathering - يعطي الذكاء وصولاً كاملاً لجميع بيانات التطبيق
...
// جمع كل البيانات للذكاء — صلاحيات كاملة
export async function gatherFullDataSummary() {
  const db = getDb();
  const now = new Date();
  ...
  try {
    ...
    const [
      ...
      db.collection('ingredients').get().catch(() => ({ docs: [] })),
      db.collection('ingredient_purchases').orderBy('createdAt', 'desc').limit(100).get().catch(() => ({ docs: [] })),
      db.collection('inventory_movements').orderBy('createdAt', 'desc').limit(50).get(),
      db.collection('users').get(),
      db.collection('ai_settings').doc('config').get(),
    ]);
